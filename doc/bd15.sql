...
select distinct
    ...
    TicketNumber
from (
    select order_id, maoyan_order_id, usermobileno as mobile, recipientidno, sellchannel, city_id, totalprice, customer_id, performance_id, meituan_userid, dianping_userid, show_name, show_id, pay_time, consumed_time, show_endtime, show_starttime, order_create_time, order_refund_status, setnumber, salesplan_count, ticket_price, province_name, city_name, ticketclass_description, detailedaddress from mart_movie.detail_myshow_saleorder where pay_time is not null and pay_time>='$$begindate' and pay_time<'$$enddate'
    and performance_id in ($performance_id)
    ) so
    left join (
    select PerformanceID as performance_id, OrderID as order_id, UserName, IDNumber, TicketNumber from origindb.dp_myshow__s_orderidentification where TicketNumber>0
