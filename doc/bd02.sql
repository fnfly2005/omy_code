...
from
    (
    select cityid, cityname from origindb.dp_myshow__s_dpcitylist where cityid is not null
    and cityname in ('北京','天津')
    ) sd
    join 
    (
    select PerformanceID, CategoryID, cityid, bsperformanceid from origindb.dp_myshow__s_performance where PerformanceID is not null
    ...
    join 
    (
    select OrderID, PerformanceName, PerformanceID, ShowName, TicketID TicketClassID, SalesPlanTicketPrice, SetNum from origindb.dp_myshow__s_ordersalesplansnapshot where OrderID is not null and CreateTime>='2016-12-31' and CreateTime<'2017-11-29'
